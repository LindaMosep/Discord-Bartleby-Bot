...
internal static class Program
{
    ...
    public static async Task MainAsync()
    {
        ...
        SignFooter = new EmbedFooterBuilder().WithText("Powered by LindaMosep!").WithIconUrl("https://media.discordapp.net/attachments/917813714923683860/923807194640687134/Grey_Cute_Illustrated_Cat_and_Fish_Circle_Laptop_Sticker_1.png?width=559&height=559");
    ...
    {
        var embed = new EmbedBuilder().WithTitle("```Please send a valid url!```").WithColor(new Color(255, 0, 0)).WithDescription("The url you sent is not valid, please send a valid url!")
    ...
    public static void GetCookies()
    {
        ...
        var msgs = _client.GetGuild(0).GetTextChannel(0).GetMessagesAsync(1).ToListAsync();
    ...
    public static async Task MessageRecieve(SocketMessage e)
    {
        ...
        if (e.Content.StartsWith("https://www.bartleby.com/questions-and-answers/"))
        {
                        ...
                        if (linksinmessage.Count > 0)
                        {
                            ...
                            if (wbc != null)
                            {
                                try
                                {
                                    ...
                                    data = data.Replace("href=\"//", "href=\"https://");
                                    data = data.Replace("href='/", "href='https://www.bartleby.com/");
                                    data = data.Replace("href=\"/", "href=\"https://www.bartleby.com/");
                                    ...
                                    if (!data.Contains("<div class=\"ErrorPage\""))
                                    {
                                        if (!data.Contains("<p id=\"pGetSolvedBtn\""))
                                        {
                                            try
                                            {
                                                var json = data.Substring(data.IndexOf("id=\"__NEXT_DATA__\" type=\"application/json\">") + "id=\"__NEXT_DATA__\" type=\"application/json\">".Length);
                                                data = data.Replace(data.Substring(data.IndexOf("id=\"__NEXT_DATA__\" type=\"application/json\">")).Remove(data.Substring(data.IndexOf("id=\"__NEXT_DATA__\" type=\"application/json\">")).IndexOf("</script>")), "");
                                                json = json.Remove(json.IndexOf("</script>"));
                                               
                                                var imageurlsno = new List<string>();
                                                var imageurls = new List<ImageUrlWithOCR>();
                                                for (int i = 0; i < 50; i++)
                                                {
                                                    if(json.IndexOf("\",\"imageUrl") != -1)
                                                    {
                                                        var imgurl = json.Substring(json.IndexOf("\",\"imageUrl\":\"") + "\",\"imageUrl\":\"".Length);
                                                        json = json.Substring(json.IndexOf("\",\"imageUrl\":\"") + "\",\"imageUrl\":\"".Length);
                                                        json = json.Substring(json.IndexOf("\",\""));
                                                        imgurl = imgurl.Remove(imgurl.IndexOf("\",\""));
                                                        var item = new ImageUrlWithOCR();
                                                        item.url = imgurl.Trim();
                                                ...
                                                doc.DocumentNode.Descendants("script").ToList().ForEach(m => m.Remove());
                                                var images = doc.DocumentNode.Descendants("img").Where(m => m.Attributes["class"] != null).Where(m => m.Attributes["class"].Value.Contains("MyQuestionLazyImage")).ToList();
                                                ...
                                                for(int i = 0; i < imageurls.Count; i++)
                                                {
                                                    if (images[i] != null)
                                                    {
                                                        ...
                                                        {
                                                            images[i].SetAttributeValue("src", imageurls[i].url);
                                                    ...
                                                    {
                                                        var myquest = doc.DocumentNode.Descendants("img").Where(m => m.Attributes["class"] != null).Where(m => m.Attributes["class"].Value.Contains("MyQuestionContainer")).ToList();
                                                        if(myquest.Count > 0)
                                                        {
                                                           var node =  HtmlAgilityPack.HtmlNode.CreateNode($"<img src=\"{imageurls[i]}\" alt=\"\"/>");
                                                            myquest[0].PrependChild(node);
                                                        }
                                                    }
                                                ...
                                                FileCount++;
                                                var divs = doc.DocumentNode.Descendants("div").Where(n => n.Attributes["class"] != null);
                                                divs.Where(m => m.Attributes["class"].Value.Contains("StickyAtTopFixedOuterContainer")).ToList().ForEach(m => m.Remove());
                                                divs.Where(m => m.Attributes["class"].Value.Contains("FeedbackUpDownButtonsContainer")).ToList().ForEach(m => m.Remove());
                                                divs.Where(m => m.Attributes["class"].Value.Contains("MyAnswerFeedbackContainer")).ToList().ForEach(m => m.Remove());
                                                divs.Where(m => m.Attributes["class"].Value.Contains("AppLayoutBaseRailWithMargins")).ToList().ForEach(m => m.Remove());
                                                divs.Where(m => m.Attributes["class"].Value.Contains("PageSectionContainerWith")).ToList().ForEach(m => m.Remove());
                                                divs.Where(m => m.Attributes["class"].Value.Contains("AppLayoutFooter")).ToList().ForEach(m => m.Remove());
                                                divs.Where(m => m.Attributes["class"].Value.Contains("styles__IconDivWrapper")).ToList().ForEach(m => m.Remove());
                                                divs.Where(m => m.Attributes["class"].Value.Contains("AppLayoutBottomContentWithoutRailsOuterWrapper")).ToList().ForEach(m => m.Remove());


                                                data  =doc.DocumentNode.OuterHtml;
                                                if(data.IndexOf("</div></div></div></div></div></div></div>") != -1)
                                                {
                                                    data = data.Insert(data.IndexOf("</div></div></div></div></div></div></div>") + "</div></div></div></div></div></div></div>".Length, footer);
                                                }



                                                File.WriteAllText(Environment.CurrentDirectory  + $@"\Meowdemia_Bartleby({count}).html", data);

                                                while (!File.Exists(Environment.CurrentDirectory  + $@"\Meowdemia_Bartleby({count}).html"))
                                                {
                                                    int c = 0;
                                                    c++;
                                                }
                                                ...
                                                var msg = await _client.GetUser(411997699647340545).SendFileAsync(Environment.CurrentDirectory + $@"\Meowdemia_Bartleby({count}).html");
                                                ...
                                                File.Delete(Environment.CurrentDirectory  +$@"\Meowdemia_Bartleby({count}).html");
